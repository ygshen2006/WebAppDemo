...
var gTransformation = {
    ...
    getNodePosition: function Transformation_getNodePosition(aNode) {
        let left=aNode.getBoundingClientRect().left;
        let top=aNode.getBoundingClientRect().top;
        let width=aNode.getBoundingClientRect().width;
        let height=aNode.getBoundingClientRect().height;

        return  {'left':left + scrollX, 'top':top + scrollY, 'width':width, 'height':height};
    },
    ...
    setSitePosition: function Transformation_setSitePosition(aSite, aPosition) {
        ...
        let top = aPosition.top;
        let left=aPosition.left;
        style.top = top + "px";
        style.left = left + "px";
    },
